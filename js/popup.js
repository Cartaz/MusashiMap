(() => {
  ...
  function renderPopup(kind, data){
    if(kind === "character"){
      return [`<strong>${data.name}</strong>`,`<br><em class="popup-location">${data.location}</em>`,data.description?`<div class="popup-description">${escapeHtml(capitalizeFirst(data.description))}</div>`:""].join("");
    }
    if(kind === "place"){
      return [`<strong>${data.name}</strong>`,`<br><em class="popup-secondary">${data.secondary}</em>`,data.description?`<div class="popup-description">${escapeHtml(data.description)}</div>`:""].join("");
    }
    if(kind === "movement"){
      return [`<strong>${data.title}</strong>`,`<br><em class="popup-secondary">${data.route}</em>`,data.description?`<div class="popup-description">${escapeHtml(capitalizeFirst(data.description))}</div>`:""].join("");
    }
    return data.content;
  }
