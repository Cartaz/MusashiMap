...
function renderCharacterFilters(selectedCharacters) {
  ...
  characterFilters.replaceChildren(...mainCharacters.map((character, index) => {
    ...
    const input = document.createElement("input"); input.type = "checkbox"; input.checked = selectedCharacters.has(character.id); input.setAttribute("aria-label", `Segui ${character.name}`);
    ...
    const name = document.createElement("span"); name.textContent = character.name; label.append(swatch, input, name); return label;
  }));
}
