...
# Single-token personal names are captured only when they occur immediately before
# a dialogue/reporting verb. This avoids treating every sentence-initial word as a name.
SINGLE_NAME_PATTERN = re.compile(
    r"\b([A-ZÀ-ÖØ-öø-ÿ][A-Za-zÀ-ÖØ-öø-ÿōūīāē'’\-]{2,})\s+(?:said|asked|replied|answered|cried|shouted|called|whispered|muttered|growled|exclaimed|continued|remarked|observed|thought|wondered|added|protested|pleaded|ordered|commanded|agreed|laughed|smiled|sighed|warned|explained|announced|declared|insisted|suggested)\b"
)
...
NON_PERSON_WORDS = {
    ...
    'The','This','That','Then','When','While','After','Before','But','And','One','Some','Many',
}
SENTENCE_STARTERS = {
...
seen = {}
def add(book, p, text, start, end, cand):
    cand = re.sub(r'\s+', ' ', cand).strip(' ,.;:!?\"“”')
    words = cand.split()
    if not words or words[0] in SENTENCE_STARTERS or any(w in NON_PERSON_WORDS for w in words):
        return
    ...
    rec['books'].add(book); rec['chapters'].add(p.stem); rec['count'] += 1
...
for book, d in BOOKS.items():
    for p in sorted(d.glob('*.txt')):
        ...
        for pat in TWO_WORD_PATTERNS:
            for m in pat.finditer(text): add(book, p, text, m.start(), m.end(), m.group(0))
        for m in SINGLE_NAME_PATTERN.finditer(text): add(book, p, text, m.start(1), m.end(1), m.group(1))

print('# Exhaustive candidate character-name audit — Books I–II\n')
print('Generated mechanically from every source chapter. Candidates are deliberately over-inclusive; classification requires source-context review. Two-word names and single-token names in dialogue attribution are scanned separately.\n')
...
for key, rec in sorted(seen.items(), key=lambda kv: (-kv[1]['count'], kv[0])):
    ...
    for c in rec['contexts']: print(f"- Context: {c}")
    print()
